...
pub fn parse_device_payload(index: u32, data: &[u8]) -> crate::core::error::Result<OpenRGBDevice> {
    let mut cursor = 0;

    let read_u16 = |cur: &mut usize| -> crate::core::error::Result<u16> {
        if *cur + 2 > data.len() { return Err(crate::core::error::LibraryError::Rgb("EOF u16".to_string())); }
        let val = u16::from_le_bytes(data[*cur..*cur+2].try_into().unwrap());
        *cur += 2;
        Ok(val)
    };

    let read_u32 = |cur: &mut usize| -> crate::core::error::Result<u32> {
        if *cur + 4 > data.len() { return Err(crate::core::error::LibraryError::Rgb("EOF u32".to_string())); }
        let val = u32::from_le_bytes(data[*cur..*cur+4].try_into().unwrap());
        *cur += 4;
        Ok(val)
    };

    let read_string = |cur: &mut usize| -> crate::core::error::Result<String> {
        let len = read_u16(cur)? as usize;
        if len == 0 { return Ok(String::new()); }
        if *cur + len > data.len() { return Err(crate::core::error::LibraryError::Rgb("EOF String".to_string())); }
    ...
    let skip_bytes = |cur: &mut usize, n: usize| -> crate::core::error::Result<()> {
        if *cur + n > data.len() { return Err(crate::core::error::LibraryError::Rgb("EOF skip_bytes".to_string())); }
        *cur += n;
        Ok(())
    };

    let skip_string = |cur: &mut usize| -> crate::core::error::Result<()> {
        let len = read_u16(cur)? as usize;
        if *cur + len > data.len() { return Err(crate::core::error::LibraryError::Rgb("EOF skip_string".to_string())); }
        *cur += len;
        Ok(())
    };
    ...
    for _ in 0..num_colors {
        if cursor + 4 > data.len() { return Err(crate::core::error::LibraryError::Rgb("EOF Colors".to_string())); }
        let r = data[cursor];
        let g = data[cursor + 1];
        let b = data[cursor + 2];
        cursor += 4;
...
pub(crate) fn find_restore_profile() -> Option<String> {
    ...
    {
        if let Some(profile) = crate::toolkit::registry::read_string(
            crate::toolkit::registry::HKEY_CURRENT_USER,
